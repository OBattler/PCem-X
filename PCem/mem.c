...
//                f=romfopen("roms/win486/win486.bin","rb");
		...
		flash_1mbit_readfiles();
                ...
                case ROM_ACERV12LC:
		f = romfopen("roms/acerv12lc/R03-B0.BIN", "rb");
                if (!f) break;
                fread(rom,           0x20000, 1, f);                
                fclose(f);
                biosmask = 0x1ffff;
		...
		flash_1mbit_readfiles();
                ...
                case ROM_430HX:
		f = romfopen("roms/430hx/0207_J2.BIN", "rb");
                if (!f) break;
                fread(rom,           0x20000, 1, f);                
                fclose(f);
                biosmask = 0x1ffff;
...
//                f=romfopen("roms/430vx/430vx","rb");               
		...
		flash_1mbit_readfiles();
                ...
                case ROM_430TX:
		f = romfopen("roms/430tx/LD5I0110.AWD", "rb");
                if (!f) break;
                fread(rom,           0x20000, 1, f);                
                fclose(f);
                biosmask = 0x1ffff;
		...
		// f = romfopen("roms/440fx/NAKX0116.AWD", "rb");		/* ASUS BIOS. */
                if (!f) break;
                fread(rom,           0x20000, 1, f);                
                fclose(f);
                biosmask = 0x1ffff;
		...
		biostype = BIOS_AMI;
                ...
                case ROM_APOLLO:
                f = romfopen("roms/apollo/S728P.ROM", "rb");
                if (!f) break;
                fread(rom,           0x20000, 1, f);                
                fclose(f);
                biosmask = 0x1ffff;
