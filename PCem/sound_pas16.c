...
static void pas16_pit_out(uint16_t port, uint8_t val, void *p)
{
        ...
        switch (port & 3)
        {
                ...
                if ((val & 0xC0) == 0xC0)
                {
                        if (!(val & 0x20))
                        {
                                if (val & 2) pas16->pit.rl[0] = pas16->pit.c[0] / (PITCONST * (1 << TIMER_SHIFT) * 3.0d);
                ...
                if (!(pas16->pit.ctrl & 0x30))
                {
                        ...
                        if (!t)
                                pas16->pit.rl[t] /= (PITCONST * (1 << TIMER_SHIFT) * 3.0d);
                        if (pas16->pit.c[t] < 0) 
                                pas16->pit.rl[t] = 0;
                        pas16->pit.ctrl |= 0x30;
                ...
                switch (pas16->pit.wm[t])
                {
                        ...
                        if (!t)
                                pas16->pit.c[t] *= PITCONST * (1 << TIMER_SHIFT) * 3.0d;
...
//        printf("Read PIT %04X ",addr);
        switch (port & 3)
        {
                ...
                if (pas16->pit.rereadlatch[t])
                {
                        ...
                        if (!t)
                        {
                                pas16->pit.rl[t] = pas16->pit.c[t] / (PITCONST * (1 << TIMER_SHIFT) * 3.0d);
...
//                pclog("pas16_pcm_poll : poll %i %i ", pas16->pit.c[0], pas16->pit.l[0]);
        ...
        {
                if (pas16->pit.l[0]) 
                        pas16->pit.c[0] += (pas16->pit.l[0] * PITCONST * (1 << TIMER_SHIFT) * 3.0d);
                else                
                        pas16->pit.c[0] += (0x10000 * PITCONST * (1 << TIMER_SHIFT) * 3.0d);
        }
        else
        {
                pas16->pit.c[0] = -1;
