...
void um8663b_write(uint16_t port, uint8_t val, void *priv)
{
        int temp;
        // pclog("um8663b_write : port=%04x reg %02X = %02X locked=%i\n", port, um8663b_curreg, val, um8663b_locked);
        if (um8663b_locked)
        {
        ...
        else
        {
                ...
                {
                        um8663b_regs[um8663b_curreg] = val;

			if (um8663b_curreg == 0xC2)
			{
				pclog("Setting drive swap to: %i (before: %i)\n", (val & 4 ? 1 : 0), drive_swap);
				fdc_setswap(val & 4 ? 1 : 0);
			}

			if (um8663b_curreg == 0xc0)
			{
	                        fdc_remove();
	                        if (um8663b_regs[0xc0] & 1)
        	                   fdc_add();
			}
                           
			if ((um8663b_curreg == 0xc0) || (um8663b_curreg == 0xc1) || (um8663b_curreg == 0xc3))
			{
	                        if (um8663b_regs[0xc0] & 2)
        	                {
	                                temp = um8663b_regs[0xc3] & 1; /*might be & 2*/
        	                        if (!(um8663b_regs[0xc1] & 2))
                	                   temp |= 2;
                        	        switch (temp)
	                                {
	                        ...
	                        if (um8663b_regs[0xc0] & 4)
        	                {
                	                temp = (um8663b_regs[0xc3] & 4) ? 0 : 1; /*might be & 8*/
                        	        if (!(um8663b_regs[0xc1] & 4))
                                	   temp |= 2;
	                                switch (temp)
        	                        {
			...
			if (um8663b_curreg == 0xc3)
			{
	                        lpt1_remove();
	                        lpt2_remove();
	                        temp = (um8663b_regs[0xc3] >> 4) & 3;
	                        switch (temp)
        	                {
...
uint8_t um8663b_read(uint16_t port, void *priv)
{
        // pclog("um8663b_read : port=%04x reg %02X locked=%i\n", port, um8663b_curreg, um8663b_locked);
        if (um8663b_locked)
           return 0xff;
        
        if (port == 0x108)
           return um8663b_curreg; /*???*/
        else
           return um8663b_regs[um8663b_curreg];
}

void um8663b_init()
{
	um8663b_regs[0xC2] = 0x81;
	/*
		The UM8699F seems to invert this compared to the PC87306.
	*/
	densel_polarity = 1;
	fdc_setswap(0);
        io_sethandler(0x0108, 0x0002, um8663b_read, NULL, NULL, um8663b_write, NULL, NULL,  NULL);
        um8663b_locked = 1;
}
